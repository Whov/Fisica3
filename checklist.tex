...
e questa è l'espressione usuale dei potenziali ritardati.
	...
	\item\c\t{Discutere un semplice apparato, di vostra scelta, per distinguere il processo
		...
		Notiamo incidentalmente che, per calcolare la distanza percorsa dai prodotti (ad esempio i pioni, che hanno vita media $\tau = 26$ $\mathrm{ns}$) ci serve un fattore gamma dato dalla dilatazione dei tempi. In particolare $\Delta L = (c \beta) * (\gamma \tau)$ con $\beta \gamma = 2.7$, ricavabile dal rapporto impulso su massa del $\pi$. Si ricava dunque che il $\pi$ percorre 21 $\mathrm{m}$ prima di decadere, dunque arriverà ai rilevatori intatto e non serve porsi il problema del $\pi$ che decade in $\mu$.\\
		Potremmo utilizzare dei calorimetri, che funzionerebbero già molto meglio del campo magnetico, al fine di misurare l'energia rilasciata dai prodotti (400 $\mathrm{Mev}$ - massa a riposo), ma possiamo fare ancora meglio.\\
		Consideriamo una lastra di un qualche materiale e supponiamo che i prodotti la attraversino. Il muone avrà perdite di energia collisionali ma essenzialmente non interagirà con la materia, mentre l'adrone avrà alta probabilità di interagire e non uscire dalla lastra. Volendo stabilire il materiale migliore per tale lastra valutiamo di minimizzare il prodotto "Lunghezza collisionale nucleare"*"Perdita di energia per collisioni" (la prima è riferita al $\pi$, la seconda a $\mu$). Queste grandezze sono tabulate sul PDG e sono riportate la prima moltiplicata per la densità, la seconda divisa per la stessa. Complessivamente si tratta quindi di calcolare il prodotto di due colonne per ogni materiale (viene fornito un breve elenco fra cui scegliere e vince la grafite che fa circa 100). Lo spessore più adatto sarà stabilito in base anche all'efficienza dei rilevatori che useremo, comunque deve essere tale da consentire l'uscita dei $\mu$ e fermare i $\pi$.
